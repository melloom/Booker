...
async function updateUserLastActive() {
    try {
        ...
        console.log('Updating last active for user:', currentUser.uid);
...
setInterval(updateUserLastActive, 60000);

// Also update when the page loads
document.addEventListener('DOMContentLoaded', () => {
    console.log('Page loaded, updating last active');
...
async function loadActiveUsers() {
    ...
    try {
        ...
        if (!currentUser) {
            ...
            showError('Please sign in to view active users');
            return;
        }
        ...
        onSnapshot(usersRef, async (snapshot) => {
            ...
            for (const doc of snapshot.docs) {
                ...
                if (roleFilter !== 'all' && userData.role !== roleFilter) {
                    console.log('User filtered out by role:', userData.role);
                    continue;
                }
                ...
                if (searchTerm) {
                    ...
                    if (!searchableText.includes(searchTerm)) {
                        console.log('User filtered out by search term');
                        continue;
                    }
                }
                ...
                const isOnline = lastActive && lastActive > activeThreshold;
                ...
                users.push({
                    ...
                    lastActive,
                    timeSinceLastActive,
                    isOnline: isCurrentUser || isOnline
                });
            }
            ...
            console.log('Active users stats:', stats);
            ...
            activeUsersList.innerHTML = users.length > 0 ? users.map(user => `
                <div class="list-item ${user.isOnline ? 'online' : 'offline'} ${user.id === currentUser.uid ? 'current-user' : ''}">
                    <div class="list-item-content">
                        <div class="list-item-title">
                            ${user.firstName} ${user.lastName}
                            ${user.id === currentUser.uid ? '<span class="current-user-badge">You</span>' : ''}
